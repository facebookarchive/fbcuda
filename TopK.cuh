...
// -finding the top-Kth float element in a set in O(N) time using radix
// selection. Uses no scratch space and does not modify inputs.
//
...
// One iteration through, by counting the distribution of
// the leading nybble in each float, we figure out what leading nybble
// the Kth highest float must have.
// As an example, let's say that K is 10.
// By counting the distribution of leading nybbles in the inputs,
// say we get:
//
// 0x2: 1 0x5: 2 0x6: 2 0x7: 11
//
// In this case, none of the floats are negative (otherwise, they
// would have leading nybble 0x8 -> 0xf). Since we're looking for
// the 10th highest float, that cannot have leading nybble 0x2, 0x5
// or 0x6 since those counts are less than 10. We walk through the
// buckets in order, and we warp reduce the counts across all
// threads to one count when it comes time to look in a particular
// bucket.
...
// count for the nybble 0x7 is not 1, we don't know the actual
// answer yet, and we have to continue.
//
...
// we have to physically scan the entire input, we are only counting
// a subset of it.
// So:
...
//
// This means that the count of all floats with the prefix
// 0x7fyyyyyy is 6. We're only bucketing counts by the second nybble
// now.
// Scanning through, the 5th highest float with prefix 0x7yyyyyyy
// must have prefix 0x7cyyyyyy, since from lowest to highest above,
// we reach 5 in bucket 0xc.
// Thus, the 10th highest float in the entire set is the unique
// float with prefix 0x7cyyyyyy. This is unique because the count
// for this bucket is 1.
...
// position `nybbleCheckPos`, but only for those ints that match
// (x & `desiredMask`) == `desired`.
// In other words, if bits [31, `nybbleCheckPos` + 4] match those in
// `desired`, then return the contents of bits
// [`nybbleCheckPos` + 3, `nybbleCheckPos`].
...
// end) without modifying the data and without temporary storage
// except for registers. K starts at 1. All threads in the warp will
// return the value. Handles all floats except NaNs. Negative zero is
// specialized by findAnswer.
// This function minimizes warp divergence.
