...
struct Complex {
  ...
  __host__ __device__ __forceinline__ Complex& operator=(const Complex& c) {
    // No need for swap
    cplx_ = c.cplx_;
    return *this;
  }

  __host__ __device__ __forceinline__ Complex operator-() const {
    return Complex(make_cuComplex(-cplx_.x, -cplx_.y));
  }

  __host__ __device__ __forceinline__ Complex operator-(const Complex& c) const {
  ...
  __host__ __device__ __forceinline__ Complex transpose() const {
  ...
  __host__ __device__ __forceinline__ void sincos(float angle) {
    sincosf(angle, &cplx_.y, &cplx_.x);
  }

  __host__ __device__ __forceinline__ float& re() {
    return cplx_.x;
  }

  __host__ __device__ __forceinline__ float& im() {
    return cplx_.y;
  }

  __host__ __device__ __forceinline__ float re() const {
    return cplx_.x;
  }

  __host__ __device__ __forceinline__ float im() const {
    return cplx_.y;
  }
